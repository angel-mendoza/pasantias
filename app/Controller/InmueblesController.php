<?php
App::uses('AppController', 'Controller');
/**
 * Inmuebles Controller
 *
 * @property Inmueble $Inmueble
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class InmueblesController extends AppController {

/**
 * Components
 *
 * @var array
 */
	public $components = array('Paginator', 'Flash', 'Session');

/**
 * index method
 *
 * @return void
 */
	public function index() {
		$this->Inmueble->recursive = 0;
		$this->set('inmuebles', $this->Paginator->paginate());
	}

/**
 * view method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function view($id = null) {
		if (!$this->Inmueble->exists($id)) {
			throw new NotFoundException(__('Invalid inmueble'));
		}
		$options = array('conditions' => array('Inmueble.' . $this->Inmueble->primaryKey => $id));
		$this->set('inmueble', $this->Inmueble->find('first', $options));
	}

/**
 * add method
 *
 * @return void
 */
	public function add() {
		if ($this->request->is('post')) {
			$this->Inmueble->create();
			if ($this->Inmueble->save($this->request->data)) {
				$this->Flash->success(__('The inmueble has been saved.'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Flash->error(__('The inmueble could not be saved. Please, try again.'));
			}
		}
	}

/**
 * edit method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function edit($id = null) {
		if (!$this->Inmueble->exists($id)) {
			throw new NotFoundException(__('Invalid inmueble'));
		}
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Inmueble->save($this->request->data)) {
				$this->Flash->success(__('The inmueble has been saved.'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Flash->error(__('The inmueble could not be saved. Please, try again.'));
			}
		} else {
			$options = array('conditions' => array('Inmueble.' . $this->Inmueble->primaryKey => $id));
			$this->request->data = $this->Inmueble->find('first', $options);
		}
	}

/**
 * delete method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function delete($id = null) {
		$this->Inmueble->id = $id;
		if (!$this->Inmueble->exists()) {
			throw new NotFoundException(__('Invalid inmueble'));
		}
		$this->request->allowMethod('post', 'delete');
		if ($this->Inmueble->delete()) {
			$this->Flash->success(__('The inmueble has been deleted.'));
		} else {
			$this->Flash->error(__('The inmueble could not be deleted. Please, try again.'));
		}
		return $this->redirect(array('action' => 'index'));
	}
}
